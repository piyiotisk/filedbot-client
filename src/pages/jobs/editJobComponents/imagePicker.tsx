...
export default class ImagePicker extends Component<Props, State> {
    ...
    componentDidUpdate = async (prevProps: Props) => {
        ...
        if (this.props.files !== prevProps.files) {
            ...
            // Override signed url so we can reupload the existing images
            const updatedFiles = await this.addSignedUrlsForExistingImages(files);
    ...
    addSignedUrlsForExistingImages = async (files: FileWithMetadata[]) => {
        const promises = files.map(async (file) => {
            console.log(file.originalCompressedFile.type)
            const data = {
                file: {
            ...
            return { ...file, imageKey, signedUrl };
        });
    ...
    fileSelectedHandler = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const existingFiles = this.state.formValues.files || [];

        const files = [...existingFiles, ...filesWithSignedUrlsAndKeys];
            ...
            <Fragment>
                <div className="mt-4 mb-4 w-full sm:w-auto">
                    <label htmlFor="images"
                        className="block text-center px-4 py-4 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150"                    >
